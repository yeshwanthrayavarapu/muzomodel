...
- 'xp.cfg': the hydra config as dumped during training. This should be used
    to rebuild the object using the muzo.models.builders functions,
...
def get_muzo_cache_dir() -> tp.Optional[str]:
    return os.environ.get('muzo_CACHE_DIR', None)
...
def _get_state_dict(
    ...
    if os.path.isdir(file_or_url_or_id):
        ...
        file = hf_hub_download(
            ...
            cache_dir=cache_dir,
            library_name="muzo",
            library_version=muzo.__version__,
        )
        return torch.load(file, map_location=device)
...
def load_lm_model_magnet(file_or_url_or_id: tp.Union[Path, str], compression_model_frame_rate: int,
    ...
    from muzo.modules.transformer import set_efficient_attention_backend
