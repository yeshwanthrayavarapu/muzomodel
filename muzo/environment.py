...
class muzoEnvironment:
    """Environment configuration for teams and clusters.

    muzoEnvironment picks compute cluster settings (slurm, dora) from the current running environment
    or declared variable and the loaded team configuration. Additionally, the muzoEnvironment
    ...
    Use the following environment variables to specify the cluster, team or configuration:
        ...
        muzo_CLUSTER (optional): Cluster type to enforce. Useful if the cluster type
            cannot be inferred automatically.
        muzo_CONFIG (optional): Path to yaml config holding the teams configuration.
            If not set, configuration is read from config/teams.yaml.
        muzo_TEAM (optional): Name of the team. Recommended to set to your own team.
    ...
    def __init__(self) -> None:
        """Loads configuration."""
        self.team: str = os.getenv("muzo_TEAM", self.DEFAULT_TEAM)
        cluster_type = _guess_cluster_type()
        cluster = os.getenv(
            "muzo_CLUSTER", cluster_type.value
        )
    ...
    def get_team(cls) -> str:
        """Gets the selected team as dictated by the muzo_TEAM env var.
    ...
    def get_cluster(cls) -> str:
        ...
        This value can be overridden by the muzo_CLUSTER env var.
        """
    ...
    def get_dora_dir(cls) -> Path:
        ...
        Value is overridden by the muzo_DORA_DIR env var.
        """
        cluster_config = cls.instance()._get_cluster_config()
        dora_dir = os.getenv("muzo_DORA_DIR", cluster_config["dora_dir"])
    ...
    def get_reference_dir(cls) -> Path:
        ...
        Value is overridden by the muzo_REFERENCE_DIR env var.
        """
        cluster_config = cls.instance()._get_cluster_config()
        return Path(os.getenv("muzo_REFERENCE_DIR", cluster_config["reference_dir"]))
