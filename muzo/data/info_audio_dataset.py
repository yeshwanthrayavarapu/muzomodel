...
def _clusterify_meta(meta: AudioMeta) -> AudioMeta:
    ...
    meta.path = muzoEnvironment.apply_dataset_mappers(meta.path)
    if meta.info_path is not None:
        meta.info_path.zip_path = muzoEnvironment.apply_dataset_mappers(meta.info_path.zip_path)
    return meta
...
class InfoAudioDataset(AudioDataset):
    ...
    See `muzo.data.audio_dataset.AudioDataset` for initialization arguments.
    """
